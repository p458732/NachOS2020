...
// from the virtual address
	...
	if (vpn >= pageTableSize) {
	   ...
	   // DEBUG(dbgAddr, "Invalid virtual page # " << virtAddr);
          ...
          j=0;
	  // 若目前memory中已有free frame則直接load進來 因此要去作線性搜尋
          while(kernel->machine->usedPhyPage[j]!=FALSE&&j<NumPhysPages){j++;}
              ...
              // 有free frame
                 if(j<NumPhysPages){ 
                      char *buf; //讀入緩衝區
                      buf = new char[PageSize];// 使buf size = page size
                  kernel->machine->usedPhyPage[j]=TRUE;// 代表要使用這塊frame
			...
			cout<<"LRUSWAP" <<endl;
		     ...
		     //作線性搜尋 尋找最不常使用的page 這裡可用heap最佳化
                     for(int index=0;index<NumPhysPages;index++){
				cout<<"page " << index<<" lastRefTime "<<pageTable[index].lastRefTime<<endl;
                             if(min > pageTable[index].lastRefTime){
                                    min = pageTable[index].lastRefTime;
                                    victim = index;
